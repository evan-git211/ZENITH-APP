...
export function SettingsModal({ onClose }: Props) {
  ...
  const saveDisplayName = async () => {
    ...
    try {
      ...
      const goal = parseInt(dailyGoalInput) || 0;
  ...
  return (
    ...
    >
      <div className="w-full max-w-xl bg-neutral-900 rounded-2xl border border-neutral-800 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      {avatarUrl && (
                        <button
                          ...
                          disabled={avatarUploading}
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm text-red-400 hover:bg-red-900/20 transition disabled:opacity-50"
                        >
                ...
                <div>
                  ...
                  <p className="text-xs text-neutral-500 mb-3">
                    Topics per day target shown in your greeting. Set 0 to disable.
                  </p>
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    {ACCENT_COLORS.map(({ id, bg, label }) => (
                      <button
                        key={id}
                        onClick={() => prefs.setAccentColor(id)}
                        title={label}
                        className="relative w-9 h-9 rounded-full transition hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-neutral-900"
                ...
                <div>
                  ...
                  <button
                    ...
                    className={`relative w-11 h-6 rounded-full transition-colors ${
                      prefs.compactMode ? 'bg-amber-500' : 'bg-neutral-700'
                    }`}
                  >
            ...
            {section === 'study' && (
              <>
                      ...
                      <div key={day.dayOfWeek} className="text-center">
                        ...
                        <div className="flex flex-col gap-1">
                          {[0, 1, 2, 3].map(w => (
                            <button
                              key={w}
                              onClick={() => updateDayWeight(day.dayOfWeek, w)}
                ...
                <div>
                  ...
                  <p className="text-xs text-neutral-500 mb-3">
                    Pre-fills the revision slider when creating new plans.
                  </p>
                  <div className="flex items-center gap-4">
                    <input
                      type="range"
                    ...
                    />
                    <span className="text-lg font-bold text-amber-400 tabular-nums w-12 text-right">
                      {localRevDays}d
                    </span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className={`relative w-11 h-6 rounded-full transition-colors ${
                        prefs.confettiEnabled ? 'bg-amber-500' : 'bg-neutral-700'
                      }`}
                    >
                      <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${
                        prefs.confettiEnabled ? 'translate-x-5' : 'translate-x-0'
                      }`} />
                    </button>
                    <Sparkles className={`w-4 h-4 ${prefs.confettiEnabled ? 'text-amber-400' : 'text-neutral-600'}`} />
                    <span className="text-sm text-neutral-400">
                      {prefs.confettiEnabled ? 'Enabled' : 'Disabled'}
                    </span>
                  </div>
                </div>
            ...
            {section === 'notifications' && (
              <>
                <div>
                  ...
                  <p className="text-xs text-neutral-500 mb-3">
                    Works while this app is open in your browser.
                  </p>
                  <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm mb-4 ${
                    ...
                    <span className={`w-2 h-2 rounded-full ${
                      notifPerm === 'granted' ? 'bg-emerald-400' : notifPerm === 'denied' ? 'bg-red-400' : 'bg-neutral-500'
                    }`} />
                ...
                <div>
                  ...
                  <p className="text-xs text-neutral-500 mb-3">
                    Get a nudge at a set time each day. Requires notifications allowed above.
                  </p>
                  <div className="flex items-center gap-3">
                    <input
                      type="time"
                  ...
                  {prefs.reminderTime && notifPerm === 'granted' && (
                    <p className="text-xs text-emerald-400 mt-2">
                      Reminder set for {prefs.reminderTime} daily.
                    </p>
                  )}
                </div>
              </>
              ...
              <>
                ...
                <div className="border-t border-red-900/40 pt-5">
                  ...
                  <button
                    ...
                    disabled={deleteConfirm !== 'DELETE' || deletingData}
