...
import {
  ...
  RotateCcw, AlarmClock, Pause, ChevronRight, Pencil, CalendarClock,
} from 'lucide-react';
...
function playAlarmSound() {
  try {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const gain = ctx.createGain();
      osc.connect(gain); gain.connect(ctx.destination);
      osc.frequency.value = i % 2 === 0 ? 1200 : 900; osc.type = 'square';
  ...
  return (
    <div className="flex gap-2 mt-1">
      <input
        ...
        className="flex-1 px-3 py-1.5 text-sm rounded-lg border border-neutral-700 bg-neutral-950 text-white focus:outline-none focus:ring-2 focus:ring-amber-500 placeholder:text-neutral-600"
      />
  ...
  if (editing) {
    return (
      <input
        ...
        className="bg-transparent border-b border-neutral-600 text-white text-sm font-semibold focus:outline-none focus:border-amber-500 px-0.5 min-w-0 w-32"
      />
    );
  }
...
export function TodoTab() {
  ...
  const [timerTick, setTimerTick] = useState(0); // incremented each second to force re-renders
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      if (timerEndTime - Date.now() <= 0) {
        ...
        playAlarmSound();
        setTimeout(() => setTimerExpired(false), 5000);
      } else {
        setTimerTick((t) => t + 1);
      }
    }, 1000);
    return () => clearInterval(id);
  }, [activeTimerTodoId, timerEndTime, timerExpired]);
  ...
  const handleClearCompleted = async () => {
    ...
    if (!confirm(`Delete ${completedIds.length} completed to-do${completedIds.length === 1 ? '' : 's'}?`)) return;
    try {
  ...
  const handleDeleteCategory = async (id: string, name: string) => {
    if (!confirm(`Delete category "${name}"? Its to-dos will move to uncategorized.`)) return;
    try {
      await deleteCategory(id);
      setCategories((prev) => prev.filter((c) => c.id !== id));
      // Move todos to uncategorized in local state (mirrors ON DELETE SET NULL)
  ...
  const handleStopTimer = useCallback(async () => {
    ...
    const elapsedMs = timerDurationMs - Math.max(0, timerEndTime - Date.now());
    if (elapsedMs > 0) {
    ...
    setTimerExpired(false);
    setTimerTick(0);
  }, [activeTimerTodoId, timerDurationMs, timerEndTime, todos]);
  ...
  const timerSecondsLeft = activeTimerTodoId && timerEndTime
  ...
  const timerProgressPercent = timerDurationMs > 0 && timerEndTime > 0
    ...
    const inner = (
      <div
        className={`flex items-center gap-3 p-3.5 rounded-xl bg-neutral-900 border transition group ${
          activeTimerTodoId === todo.id
            ? 'border-blue-600 shadow-md shadow-blue-900/20'
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-sm font-medium truncate ${todo.is_completed ? 'text-neutral-500 line-through' : 'text-white'}`}>
            {todo.title}
          </p>
            ...
            {showDeadlinePicker === todo.id && (
              ...
              >
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 text-sm rounded-lg border border-neutral-700 bg-neutral-950 text-white focus:outline-none focus:ring-2 focus:ring-amber-500 mb-2"
                />
                {/* Time toggle */}
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {dpHasTime && (
                    <input
                      ...
                      className="flex-1 px-3 py-1 text-sm rounded-lg border border-neutral-700 bg-neutral-950 text-white focus:outline-none focus:ring-2 focus:ring-amber-500"
                    />
                  )}
                </div>
                {/* Apply / Clear */}
        ...
        {!todo.is_completed && activeTimerTodoId !== todo.id && (
          <div className="relative flex-shrink-0" onClick={(e) => e.stopPropagation()}>
            <button
              ...
              className="p-1.5 rounded-lg text-blue-400 hover:bg-blue-900/20 opacity-0 group-hover:opacity-100 transition disabled:opacity-30"
              title="Set timer"
            >
              ...
              >
                ...
                <div className="grid grid-cols-4 gap-1.5 mb-3">
                  {TIMER_PRESETS.map((min) => (
                    <button key={min} onClick={() => handleStartTimer(todo.id, min)}
                      className="py-1.5 rounded-lg text-sm font-medium bg-blue-900/30 text-blue-300 hover:bg-blue-900/50 transition">
                      {min}m
                    </button>
                  ))}
                </div>
                <div className="flex gap-2">
                  <input type="number" min={1} max={180} value={customMinutes}
                    ...
                    className="flex-1 px-3 py-1.5 text-sm rounded-lg border border-neutral-700 bg-neutral-950 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder:text-neutral-600"
                  ...
                  <button onClick={() => customMinutes && handleStartTimer(todo.id, parseInt(customMinutes))}
                    ...
                    className="px-3 py-1.5 rounded-lg bg-blue-600 text-white text-sm hover:bg-blue-500 disabled:opacity-50 transition">
                    Go
                  </button>
                </div>
              </div>
    ...
    <div onClick={() => { setShowTimerPicker(null); setShowDeadlinePicker(null); }}>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-neutral-400 border border-neutral-700 hover:bg-neutral-800 hover:text-white transition"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-medium transition"
          >
            <Plus className="w-4 h-4" />
            Add To-Do
          </button>
        ...
        <div className="mb-4 p-4 rounded-xl bg-neutral-900 border border-neutral-800">
          <div className="flex gap-3">
            <input autoFocus type="text" value={newTodoTitle}
              ...
              className="flex-1 px-4 py-2 rounded-lg border border-neutral-700 bg-neutral-950 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder:text-neutral-600 text-sm"
              onKeyDown={(e) => { if (e.key === 'Enter') handleAddTodo(newTodoTitle); if (e.key === 'Escape') { setShowTodoForm(false); setNewTodoTitle(''); } }}
            />
            <button onClick={() => handleAddTodo(newTodoTitle)} className="px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-500 transition">
              <Check className="w-4 h-4" />
            </button>
      ...
      {activeTimerTodoId && (
        <div className={`mb-4 p-4 rounded-xl border transition-all ${
          timerExpired ? 'bg-red-900/30 border-red-600 animate-pulse' : 'bg-blue-900/20 border-blue-800'
        }`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={`w-3 h-3 rounded-full ${timerExpired ? 'bg-red-500' : 'bg-blue-500 animate-pulse'}`} />
              <div>
                <span className={`font-semibold text-sm ${timerExpired ? 'text-red-400' : 'text-white'}`}>
                  {timerExpired ? "Time's up!" : todos.find((t) => t.id === activeTimerTodoId)?.title}
                </span>
                {!timerExpired && (
                  <div className="mt-1 h-1.5 bg-blue-900/50 rounded-full overflow-hidden w-40">
                    <div className="h-full bg-blue-500 transition-all duration-1000" style={{ width: `${timerProgressPercent}%` }} />
                  </div>
                )}
              </div>
            </div>
            <div className="flex items-center gap-3">
              {timerExpired
                ...
                : <span className="text-2xl font-mono font-bold text-blue-400 tabular-nums">{formatCountdown(timerSecondsLeft)}</span>
              }
              <button onClick={handleStopTimer} className={`p-2 rounded-lg text-white ${timerExpired ? 'bg-red-600 hover:bg-red-500' : 'bg-blue-600 hover:bg-blue-500'}`}>
      ...
      {!loading && todos.length === 0 && categories.length === 0 && (
        <div className="text-center py-12">
          <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-12 max-w-md mx-auto">
            ...
            <h3 className="text-lg font-semibold text-white mb-2">No to-dos yet</h3>
            <p className="text-neutral-400 mb-6">Add to-dos or create categories to organize your work</p>
            <div className="flex gap-3 justify-center">
              <button onClick={() => setShowTodoForm(true)} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-medium transition text-sm">
        ...
        <DragDropContext onDragEnd={handleDragEnd}>
          <div className="space-y-4">
            ...
            {(() => {
              ...
              return (
                <div className={`rounded-xl border border-neutral-800 overflow-hidden ${categories.length === 0 ? '' : ''}`}>
                  {categories.length > 0 && (
